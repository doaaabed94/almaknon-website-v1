...
class CurrencyService extends BaseService
{
    ...
    public function getModel($id, ...$relations)
    {
        ...
        if (auth()->user()->isAn('ROOT') or auth()->user()->can('RESTORE_CURRENCY')) {
            $this->data['model']->withTrashed();
        }

        if (auth()->user()->isAn('ROOT') or auth()->user()->can('STATUS_UPDATE_CURRENCY')) {
    ...
    public function store($data)
    {
        ...
        $this->data['model']->code   = $this->data['store_data']['code'];
        $this->data['model']->symbol = $this->data['store_data']['symbol'];

        $this->data['model']->status = empty($this->data['store_data']['status']) ? 'ACTIVE' : $this->data['store_data']['status'];
        $this->data['model']->save();

        return $this->response(200, [], null, route('currency.index'));
    }
    ...
    public function update($id, $data)
    {
        ...
        // }
        $this->data['model']->code       = $this->data['update_data']['code'];
        $this->data['model']->symbol     = $this->data['update_data']['symbol'];
        $this->data['model']->created_by = $this->data['currentUser']->id;
        $this->data['model']->save();

        return $this->response(200, [], null, route('currency.update', ['model' => $id]));
    }
