...
use Modules\Academic\Services\DegreeService;
use Modules\Academic\Entities\Degree;
use Validator;
use Exception;
...
class DegreeController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('NeedPermissions:READ_DEGREES')->only(['index', 'postIndex', 'read']);
        $this->middleware('NeedPermissions:CREATE_DEGREES')->only(['create', 'postCreate']);
        $this->middleware('NeedPermissions:UPDATE_DEGREES')->only(['update', 'postUpdate']);
        $this->middleware('NeedPermissions:DELETE_DEGREES')->only(['postDelete']);
        $this->middleware('NeedPermissions:RESTORE_DEGREES')->only(['postRestore']);
        $this->middleware('NeedPermissions:PERMA_DELETE_DEGREES')->only(['postPermaDelete']);
        $this->middleware('NeedPermissions:STATUS_UPDATE_DEGREES')->only(['postStatus']);

        $this->service = new DegreeService();
        $this->validationsRules = $this->service->validationsRules;
        $this->data['model'] = Degree::query();
    }

    public function index(Request $request)
    {
        $this->data['all_data'] = $this->service->index();
        return view('maknon::degrees.index', $this->data);
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('maknon::degrees.create', $this->data);
    }
    ...
    public function update(Request $request)
    {
        ...
        return view('maknon::degrees.update', $this->data);
    }
    ...
    public function read(Request $request)
    {
        ...
        return view('maknon::degrees.read', $this->data);
    }
}
