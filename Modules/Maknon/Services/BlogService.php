...
use Modules\Maknon\Entities\Blog;
...
class BlogService extends BaseService
{
    ...
    {
        $this->data['model'] = Blog::query();

        if (auth()->user()->isAn('ROOT') or auth()->user()->can('RESTORE_blogS'))
            $this->data['model']->withTrashed();

        if (auth()->user()->isAn('ROOT') or auth()->user()->can('STATUS_UPDATE_blogS'))
    ...
    public function store($data)
    {
        ...
        $this->data['model']->type                          = $this->data['store_data']['type'];
        ...
        return $this->response(200, [], null, route('blogs.index'));
    }
    ...
    public function update($id, $data)
    {
        ...
        $this->data['model']->type                          = $this->data['update_data']['type'];
        $this->data['model']->created_by                    = $this->data['currentUser']->id;
        $this->data['model']->save();


        return $this->response(200, [], null, route('blogs.update', ['model' => $id]));
    }
